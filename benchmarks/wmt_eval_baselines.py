...
def run_sentence_chrf(candidates: list, references: list) -> list:
    ...
    for i in tqdm(range(len(candidates)), desc="Running chrF..."):
        chrf_scores.append(sentence_chrf(hypothesis=candidates[i], reference=references[i]).score)
    return chrf_scores
...
if __name__ == "__main__":
    ...
    if args.run_wmt18:
        ...
        for lp in lps:
            ...
            print(f"SentenceBLEU Kendall for {lp} : {kendall}")
            ...
            kendall = compute_kendall(scores_hyp1, scores_hyp2, data)
            print(f"chrF Kendall for {lp} : {kendall}\n")
