...
class TestRankingMetric(unittest.TestCase):
    ...
    def test_training(self):
        ...
        trainer.fit(model)
        self.assertTrue(
            os.path.exists(os.path.join(DATA_PATH , "checkpoints", "epoch=3-step=15.ckpt"))
        )
        saved_model = RankingMetric.load_from_checkpoint(
            os.path.join(DATA_PATH , "checkpoints", "epoch=3-step=15.ckpt")
        )
        dataset = saved_model.read_csv(os.path.join(DATA_PATH, "test_regression_data.csv"), regression=True)
        ...
        y_hat = torch.cat(trainer.predict(dataloaders=dataloader, return_predictions=True), dim=0).cpu().tolist()
