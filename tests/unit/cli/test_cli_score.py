...
class TestScoreCli(unittest.TestCase):
    ...
    def test_score_estimator_cpu(self):
        ...
        args = [
            "--model", DATA_PATH+"hter-estimator-v1.0/_ckpt_epoch_1.ckpt", 
            "-s", DATA_PATH+"src.en", 
            "-h", DATA_PATH+"mt.de", 
            "-r", DATA_PATH+"ref.de", 
            "--to_json",  DATA_PATH+"results.json"
            "--cpu", 
        ]
        result = self.runner.invoke(score, args, catch_exceptions=False)
        self.assertEqual(result.exit_code, 0)
        self.assertIn("Predictions saved in: {}".format(DATA_PATH+"results.json"), result.stdout)
        self.assertIn("COMET system score: ", result.stdout)
