...
def calculate_bootstrap(
    x_sys_scores: np.ndarray,
    y_sys_scores: np.ndarray,
    x_name: Path_fr,
    y_name: Path_fr,
...
def pairwise_bootstrap(
    ...
    """
    assert sys_scores.shape[0] == len(
        systems
    ), 'Each system should have its sys_score.'
...
def score(cfg: Namespace, systems: List[Dict[str, List[str]]]) -> np.ndarray:
    ...
    seg_scores = np.array(
        seg_scores, dtype='float32'
    )  # num_systems x num_translations
    return seg_scores
...
def get_cfg() -> Namespace:
    ...
    parser.add_argument(
        '--only_system',
        action='store_true',
        help='Prints only the final system score.',
    )
    parser.add_argument(
        '--num_splits',
        type=int,
    ...
    if cfg.sources is None and cfg.sacrebleu_dataset is None:
        parser.error(
            f'You must specify a source (-s) or a sacrebleu dataset (-d)'
        )
...
def compare_command() -> None:
    ...
    if model.requires_references() and (cfg.references is None):
        parser.error(
            '{} requires -r/--references or -d/--sacrebleu_dataset.'.format(
                cfg.model
            )
        )
    ...
    assert (
        len(cfg.translations) > 1
    ), 'You must provide at least 2 translation files'
    ...
    if cfg.references is not None:
        ...
        systems = [
            {'src': sources, 'mt': system, 'ref': references}
    ...
    info = {
        ...
        'translations': [
            ...
            for name, trans, scores in zip(
                cfg.translations, translations, seg_scores
            )
        ],
    }
